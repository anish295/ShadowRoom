...
const CHUNK_SIZE = 256 * 1024; // 256KB chunk size (higher throughput for large files)
const MAX_BUFFERED_AMOUNT = 8 * 1024 * 1024; // 8MB soft limit for backpressure
...
async function deriveAesKey(pin) {
  if (!pin || typeof window === "undefined" || !window.crypto?.subtle) {
    return null;
  }
  const enc = new TextEncoder();
  const salt = enc.encode("shadowroom-aes-salt-v1");
...
export function createWebRtcSession({
  ...
  const sendJson = async (obj) => {
    ...
    if (aesKey) {
      try {
        ...
        const encrypted = await encryptBuffer(aesKey, new TextEncoder().encode(str));
        const payload = { t: "aes-json", d: arrayBufferToBase64(encrypted) };
  ...
  const sendBinary = async (ab) => {
    ...
    if (aesKey) {
      try {
        const encrypted = await encryptBuffer(aesKey, ab instanceof ArrayBuffer ? ab : ab.buffer || ab);
        peer.send(encrypted);
        return;
      } catch (err) {
        // fallback to plain send if encryption fails
      }
    }
    peer.send(ab);
  };
  ...
  const setupPeer = () => {
    ...
    peer.on("data", async (data) => {
      ...
      if (str != null) {
        let msg = safeJsonParse(str);
        if (!msg?.t) return;

        if (msg.t === "aes-json") {
          if (!aesKey || typeof msg.d !== "string") return;
          try {
            const decrypted = await decryptBuffer(aesKey, base64ToArrayBuffer(msg.d));
            const full = new TextDecoder().decode(decrypted);
            const inner = safeJsonParse(full);
            if (inner && inner.t) {
              // Re-use message handling with decrypted payload
              msg = inner;
            } else {
              return;
            }
        ...
        if (msg.t === "file-meta") {
          const canSaveToDisk = canUseFileSystemAccessApi();
          const shouldAutoStream = canSaveToDisk && msg.size >= 200 * 1024 * 1024; // 200MB+
          ...
          incoming.set(msg.id, {
            ...
            expectedSeq: 0,
            startedAt: Date.now(),
            mode: shouldAutoStream ? "stream" : "memory",
            writer: null,
            handle: null,
          });
          ...
          if (shouldAutoStream) {
            saveIncomingToDisk(msg.id).catch(() => {
              // Fall back to memory if user cancels/save fails.
      ...
      if (aesKey) {
        try {
          const raw = data instanceof Uint8Array ? data : new Uint8Array(data instanceof ArrayBuffer ? data : data.buffer);
          const dec = await decryptBuffer(aesKey, raw);
          decryptedData = new Uint8Array(dec);
        } catch {
          // if fail, assume data is plain chunk
          decryptedData = data;
        }
      }

      const u8 = decryptedData instanceof Uint8Array
        ? decryptedData
        : new Uint8Array(decryptedData instanceof ArrayBuffer ? decryptedData : decryptedData.buffer);
      if (entry.mode === "stream" && entry.writer) {
        try {
